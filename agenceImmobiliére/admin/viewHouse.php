<?php
    require 'connexion.php';
    if (!empty($_GET['code']))
    {
        $code = checkInput($_GET['code']);
    }
    $db = Database::connect();
    $statement = $db->prepare(" SELECT * FROM maison WHERE code = ?");
    $statement->execute(array($code));
    $ms = $statement->fetch();
    function checkInput($data)
    {
        $data = trim($data);
        $data = htmlspecialchars($data);
        $data = stripslashes($data);
        return $data;
    }
?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title> Projet Fin d'etude </title>
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css">
        <script src="https://stackpath.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js"></script>
        <link rel="stylesheet" href="../css/styles.css">
        <link rel="stylesheet" type="text/css" href="//fonts.googleapis.com/css?family=Holtwood+One+SC" />
    </head>
    <body>
        <h1 class="text-logo"><span class="glyphicon glyphicon-home"></span> Site d'agence immobiliére <span class="glyphicon glyphicon-home"></span></h1>
        <div class="container admin">
            <div class="row">
                <h1><strong> Voir une maison </strong></h1>
                <br>
                <div class="col-md-6">
                    <form>
                        <div class="form-group">
                            <label for="code"> Code: </label> <?php echo ' ' . $ms['code']; ?>
                        </div>
                        <div class="form-group">
                            <label for="region"> Region: </label> <?php echo ' ' . $ms['region']; ?>
                        </div>
                        <div class="form-group">
                            <label for="nombre_de_chambre"> Nombre de chambre: </label> <?php echo ' ' . $ms['nombre_de_chambre']; ?>
                        </div>
                        <div class="form-group">
                            <label for="prix_de_location"> Prix de location: </label> <?php echo ' ' . $ms['prix_de_location'] . ' UM '; ?>
                        </div>
                        <div class="form-group">
                            <label for="type"> Etat: </label> <?php echo '  ' . $ms['type']; ?>
                        </div>
                        <div class="form-actions">
                            <a href="index.php" class="btn btn-primary"><span class="glyphicon glyphicon-arrow-left"></span> Retour</a>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </body>
</html>
